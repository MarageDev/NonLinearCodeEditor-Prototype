...
func _ready() -> void:
	...
	if ResourceLoader.exists(GlobalManager.SavedColorSwatchesResourcePath):
		var res: SaveColorSwatchesResource = load(GlobalManager.SavedColorSwatchesResourcePath)
		for swatch in res.swatches:
			var btn =  preload("res://UI_Elements/ColorWheel/ColorSwatchedButton/ColorSwatchesButton.tscn").instantiate()
...
func _on_button_pressed() -> void:
	var newColorSwatchesButton := preload("res://UI_Elements/ColorWheel/ColorSwatchedButton/ColorSwatchesButton.tscn").instantiate()
	...
	var swatch_resource: SaveColorSwatchesResource
	if ResourceLoader.exists(GlobalManager.SavedColorSwatchesResourcePath):
		swatch_resource = load(GlobalManager.SavedColorSwatchesResourcePath)
	else:
		swatch_resource = SaveColorSwatchesResource.new()  # Create new if doesn't exist

	var swatch = ColorSwatcheRessource.new()
